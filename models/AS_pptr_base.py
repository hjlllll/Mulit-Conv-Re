...
class PrimitiveTransformer(nn.Module):
    def __init__(self, radius, nsamples, spatial_stride,                                # P4DConv: spatial
        ...
        # self.tube_embedding2 = P4DConv(in_planes=0, mlp_planes=[dim], mlp_batch_norm=[False], mlp_activation=[False],
        #                           spatial_kernel_size=[int(radius**0.5), nsamples], spatial_stride=int(spatial_stride**0.5),
        #                           temporal_kernel_size=temporal_kernel_size, temporal_stride=temporal_stride, temporal_padding=[1, 1],
        #                           operator='+', spatial_pooling='max', temporal_pooling='max')
        # self.tube_embedding3 = P4DConv(in_planes=0, mlp_planes=[dim], mlp_batch_norm=[False], mlp_activation=[False],
        #                           spatial_kernel_size=[int(radius**0.5), nsamples], spatial_stride=int(spatial_stride**0.5),
        #                           temporal_kernel_size=temporal_kernel_size, temporal_stride=temporal_stride, temporal_padding=[1, 1],
        #                           operator='+', spatial_pooling='max', temporal_pooling='max')
        ...
        # self.mlp = nn.Linear(81,64)
        self.nor = nn.LayerNorm(dim)
        self.con1d = nn.Linear(8,1)

    def forward(self, input):



        # xyzs, features = self.tube_embedding2(input)  # [B, L, n, 3], [B, L, C, n]
        # xyzs, features = self.tube_embedding3(xyzs)  # [B, L, n, 3], [B, L, C, n]
        # xyzs = self.mlp(xyzs.transpose(2,3)).transpose(2,3)
...
#         max_values, max_indices = torch.max(features, dim=2)
#         f1 = torch.gather(features, 2, max_indices.unsqueeze(2)).squeeze(2)
#         f1 = self.nor(f1)
...
#         output =  self.mlp_head(primitive_feature + f1)
