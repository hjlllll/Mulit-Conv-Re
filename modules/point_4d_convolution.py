    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames
            ...
            for i in range(t-self.temporal_kernel_size//2, t+self.temporal_kernel_size//2+1):
                ...
                ###################################
                idx2 = pointnet2_utils.ball_query(self.r*2, self.k, neighbor_xyz, anchor_xyz)
                neighbor_xyz_flipped2 = neighbor_xyz.transpose(1, 2).contiguous()                                                    # (B, 3, N)
                neighbor_xyz_grouped2 = pointnet2_utils.grouping_operation(neighbor_xyz_flipped2, idx2)                                # (B, 3, N//spatial_stride, k)
                xyz_displacement2 = neighbor_xyz_grouped2 - anchor_xyz_expanded 
                # xyz_displacement += xyz_displacement2
                t_displacement2 = torch.ones((xyz_displacement2.size()[0], 1, xyz_displacement2.size()[2], xyz_displacement2.size()[3]), dtype=torch.float32, device=device) * (i-t)
                displacement2 = torch.cat(tensors=(xyz_displacement2, t_displacement2), dim=1, out=None)                               # (B, 4, N//spatial_stride, k)
                displacement2 = self.conv_d(displacement2)
                if self.in_planes != 0:
                    neighbor_feature_grouped = pointnet2_utils.grouping_operation(features[i], idx2)                                 # (B, in_planes, N//spatial_stride, k)
                    feature2 = self.conv_f(neighbor_feature_grouped)
                    if self.operator == '+':
                        feature2 = feature2 + displacement2
                    else:
                        feature2 = feature2 * displacement2
                else:
                    feature2 = displacement2
                    
                idx3 = pointnet2_utils.ball_query(self.r*4, self.k, neighbor_xyz, anchor_xyz)
                neighbor_xyz_flipped3 = neighbor_xyz.transpose(1, 2).contiguous()                                                    # (B, 3, N)
                neighbor_xyz_grouped3 = pointnet2_utils.grouping_operation(neighbor_xyz_flipped3, idx3)                                # (B, 3, N//spatial_stride, k)
                xyz_displacement3 = neighbor_xyz_grouped3 - anchor_xyz_expanded 
                # xyz_displacement += xyz_displacement2
                t_displacement3 = torch.ones((xyz_displacement3.size()[0], 1, xyz_displacement3.size()[2], xyz_displacement3.size()[3]), dtype=torch.float32, device=device) * (i-t)
                displacement3 = torch.cat(tensors=(xyz_displacement3, t_displacement3), dim=1, out=None)                               # (B, 4, N//spatial_stride, k)
                displacement3 = self.conv_d(displacement3)
                if self.in_planes != 0:
                    neighbor_feature_grouped = pointnet2_utils.grouping_operation(features[i], idx3)                                 # (B, in_planes, N//spatial_stride, k)
                    feature3 = self.conv_f(neighbor_feature_grouped)
                    if self.operator == '+':
                        feature3 = feature3 + displacement3
                    else:
                        feature3 = feature3 * displacement3
                else:
                    feature3 = displacement3
                    
                idx4 = pointnet2_utils.ball_query(self.r*6, self.k, neighbor_xyz, anchor_xyz)
                neighbor_xyz_flipped4 = neighbor_xyz.transpose(1, 2).contiguous()                                                    # (B, 3, N)
                neighbor_xyz_grouped4 = pointnet2_utils.grouping_operation(neighbor_xyz_flipped4, idx4)                                # (B, 3, N//spatial_stride, k)
                xyz_displacement4 = neighbor_xyz_grouped4 - anchor_xyz_expanded 
                # xyz_displacement += xyz_displacement2
                t_displacement4 = torch.ones((xyz_displacement4.size()[0], 1, xyz_displacement4.size()[2], xyz_displacement4.size()[3]), dtype=torch.float32, device=device) * (i-t)
                displacement4 = torch.cat(tensors=(xyz_displacement4, t_displacement4), dim=1, out=None)                               # (B, 4, N//spatial_stride, k)
                displacement4 = self.conv_d(displacement4)
                if self.in_planes != 0:
                    neighbor_feature_grouped = pointnet2_utils.grouping_operation(features[i], idx4)                                 # (B, in_planes, N//spatial_stride, k)
                    feature4 = self.conv_f(neighbor_feature_grouped)
                    if self.operator == '+':
                        feature4 = feature4 + displacement4
                    else:
                        feature4 = feature4 * displacement4
                else:
                    feature4 = displacement4
                
                # feature2 = self.mlp(feature2)
                # feature = torch.cat((feature, feature2, feature3), dim=3)
