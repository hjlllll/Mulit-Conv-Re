...
from datasets.AS_base_test import SegDatasetTest
...
def evaluate(model, criterion, data_loader, device, len_test):
    ...
    with torch.no_grad():
        ...
        length = 0
        outputs = []
        for clip in metric_logger.log_every(data_loader, 20, header):
...
#             output = output.cpu().numpy().astype(np.int32)
        outputss = torch.cat(outputs, dim=0)
    return outputss
#             acc = np.mean(output == target)
            # acc = torch.mean(torch.tensor(output==target,dtype=torch.float))
#             acc_list.append(acc)
...
#     dataset = SegDataset(root='../autodl-tmp', train=True)

    dataset_test = SegDatasetTest(root='../autodl-tmp', train=False)
...
#     lr_scheduler = WarmupMultiStepLR(optimizer, milestones=lr_milestones, gamma=args.lr_gamma, warmup_iters=warmup_iters, warmup_factor=1e-5)
    ...
    start_time = time.time()
    acc = 0
    out = evaluate(model, criterion, data_loader_test, device, len(dataset_test))
    outf = out.cpu().numpy().astype(np.int64)
    np.save('pred',outf)
#     a = np.load('pred.npy')
#     print(a.shape, a)
...
def parse_args():
    ...
    # resume
    parser.add_argument('--resume', default='output/model_49.pth', help='resume from checkpoint')
